...
import { alertConfirm, alertError, alertSuccess } from "../../lib/alart";
...
export default function ContactList() {
    ...
    useEffect(() => {
        fetchContactList()
        .then(() => console.log('Contact list fetched successfully'));
    }, [reload]);
        ...
        <>
            <div>
                ...
                <div className="mt-10 flex justify-center">
                    ...
                    {page > 1 && 
                    <button onClick={() => handlePageChange(page - 1)} className="px-4 py-2 bg-gray-700 text-gray-300 rounded-lg hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-800 transition-all duration-200 flex items-center">
                        <i className="fas fa-chevron-left mr-2" /> Previous
                    </button>
                    }
                    {getPages().map((value) => {
                        if(value === page){
                            return <button key={value} className="px-4 py-2 bg-gradient text-white rounded-lg hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-800 transition-all duration-200 font-medium shadow-md">
                                {value}
                            </button>
                        }else{
                            return <button key={value} onClick={() => handlePageChange(value)} className="px-4 py-2 bg-gray-700 text-gray-300 rounded-lg hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-800 transition-all duration-200">
                                {value}
                            </button>
                        }
                    })}
