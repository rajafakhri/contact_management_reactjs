...
export default function ContactEdit() {
    ...
    async function fetchContactDetail() {
        ...
        if(response.status === 200) {
            setFirstName(reponseBody.data.first_name);
            setLastName(reponseBody.data.last_name);
            setEmail(reponseBody.data.email);
            setPhone(reponseBody.data.phone);
        }else{
    ...
    async function handleSubmit(e){
        e.preventDefault();
        const response = await contactUpdate({token, id, first_name: firstName, last_name: lastName, email, phone});
        const reponseBody = await response.json();
        console.log(reponseBody)
        if(response.status === 200) {
