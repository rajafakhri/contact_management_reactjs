...
import { userDetail, userPassword, userProfile } from "../../lib/api/UserApi";
import { alertError, alertSuccess } from "../../lib/alart";
...
export default function UserProfile() {
    ...
    async function handleSubmitProfile(e){
        e.preventDefault();
        const response = await userProfile(token, {name});
        const reponseBody = await response.json();
        console.log(reponseBody);
        if(response.status === 200) {
    ...
    async function handleSubmitPassword(e){
        ...
        const response = await userPassword(token, {password});
        const reponseBody = await response.json();
        console.log(reponseBody);
        if(response.status === 200) {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="p-6">
                        ...
                        <div className="mb-5">
                            ...
                            <input type="password" id="new_password" name="new_password" className="w-full pl-10 pr-3 py-3 bg-gray-700 bg-opacity-50 border border-gray-600 text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200" placeholder="Enter your new password" value={password} onChange={(e) => setPassword(e.target.value)} required />
                            </div>
                        </div>
                        <div className="mb-5">
                            ...
                            <input type="password" id="confirm_password" name="confirm_password" className="w-full pl-10 pr-3 py-3 bg-gray-700 bg-opacity-50 border border-gray-600 text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200" placeholder="Confirm your new password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required />
                            </div>
                        </div>
                        <div className="mt-6">
                            <button type="submit" className="w-full bg-gradient text-white py-3 px-4 rounded-lg hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-800 transition-all duration-200 font-medium shadow-lg transform hover:-translate-y-0.5 flex items-center justify-center">
